  ...
  return (
    <div className="space-y-3">
      {alerts.map((alert, idx) => (
        <div
          key={idx}
          className="rounded-2xl border-2 border-amber-400 bg-gradient-to-r from-amber-50 via-amber-50/80 to-orange-50/50 p-5 sm:p-6 shadow-md"
        >
          <div className="flex items-start space-x-3.5">
            ...
            <div className="flex-1">
              <div className="flex flex-wrap items-center justify-between gap-2 mb-1">
                <span className="inline-flex items-center text-xs font-extrabold uppercase tracking-wider text-amber-800 bg-amber-200/80 px-2 py-0.5 rounded">
                  <ShieldAlert className="w-3.5 h-3.5 mr-1" />
                  VERSION INTELLIGENCE ALERT • SUPERSEDED STANDARD DETECTED
                </span>
              ...
              <h4 className="text-sm sm:text-base font-bold text-slate-900 mt-1 mb-2">
                You explicitly referenced a superseded edition of an Indian Standard.
              </h4>
              ...
              <p className="text-xs text-slate-700 leading-relaxed">
                {alert.message} In accordance with standard procurement norms, specifications should cite current active editions unless explicitly executing legacy maintenance contracts.
              </p>

              <div className="mt-4 pt-3 border-t border-amber-200/80 flex flex-wrap items-center justify-between gap-3 text-xs">
                <span className="text-slate-600">
                  ManakSetu has retained your original reference while highlighting the active edition.
                </span>
                <Link
                  to="/graph"
                  className="inline-flex items-center font-bold text-amber-900 hover:text-amber-950 bg-amber-200/90 hover:bg-amber-300 px-3 py-1.5 rounded-lg transition"
                >

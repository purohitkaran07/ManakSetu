...
export const AnalyzerTabs: React.FC<AnalyzerTabsProps> = ({
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200 shadow-xl shadow-slate-900/5 overflow-hidden">
      {/* Top Tabs matching Reference 2 */}
      <div className="flex border-b border-slate-200/90 bg-white overflow-x-auto text-xs font-semibold scrollbar-none px-2 pt-1">
        <button
      ...
      <div className="p-5">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="relative">
              ...
              <textarea
                ...
                className="w-full rounded-xl border border-slate-200/90 bg-white p-3.5 text-sm text-slate-800 placeholder-slate-400 focus:border-[#0067C5] focus:ring-2 focus:ring-blue-100 outline-none transition resize-none leading-relaxed"
              />
            ...
            <div>
              <button
                ...
                className="flex items-center text-xs font-semibold text-slate-600 hover:text-blue-700 transition"
              >
              ...
              {showAdvanced && (
                <div className="mt-3 p-3.5 bg-slate-50 border border-slate-200 rounded-xl space-y-2 text-xs">
                  ...
                  <select
                    ...
                    className="w-full bg-white border border-slate-300 rounded-lg px-2.5 py-1.5 text-xs text-slate-700 focus:outline-none focus:ring-2 focus:ring-blue-500"
                  >
                    <option value="none">Deterministic Rule & Pattern Engine (Default, Zero-API-Key)</option>
                    <option value="gemini">Gemini LLM Analyzer (Optional Cloud Integration)</option>
                  </select>
                  <p className="text-[11px] text-slate-500">
                    Deterministic engine operates offline without external API dependencies.
                  </p>
                </div>
              )}
            </div>

            {/* Bottom Actions matching Reference 2 */}
            <div className="flex items-center justify-end pt-2 border-t border-slate-100">
              <button
                type="submit"
                disabled={isLoading || !requirementText.trim()}
                className={`inline-flex items-center justify-center px-6 py-2.5 rounded-xl font-bold text-sm text-white transition shadow-sm ${
                  isLoading || !requirementText.trim()
                    ? 'bg-slate-300 cursor-not-allowed text-slate-500 shadow-none'
                    : 'bg-[#0067C5] hover:bg-[#00529B] shadow-blue-500/10'
                }`}
              >
                {isLoading ? (
                  <>
        ...
        {activeTab === 'upload' && (
          <div className="text-center py-8 px-4 border-2 border-dashed border-slate-200 rounded-xl bg-slate-50/50">
            ...
            <h4 className="text-sm font-bold text-slate-800 mb-1">Tender Document Parsing</h4>
            <p className="text-xs text-slate-500 max-w-sm mx-auto mb-4">
              Upload PDF or DOCX procurement specifications (planned for Phase 2). For the live prototype, use natural-language text.
            </p>
            <button
              onClick={() => setActiveTab('describe')}
              className="inline-flex items-center text-xs font-semibold text-blue-600 hover:text-blue-700 bg-blue-50 px-3 py-1.5 rounded-lg border border-blue-200"
            >
          ...
          <div className="space-y-4">
            <div className="relative">
              ...
              <input
                ...
                placeholder="Type a product name (e.g. Water Heater, Geyser)..."
            ...
            <div>
              <p className="text-xs font-medium text-slate-500 mb-2">Common Products in Knowledge Base:</p>
              <div className="flex flex-wrap gap-2">
                {[
                  ...
                  <button
                    ...
                    className="text-xs font-medium px-3 py-1.5 rounded-lg bg-slate-100 hover:bg-blue-50 text-slate-700 hover:text-blue-700 border border-slate-200 transition"
                  >
                    + {prod}
                  </button>
                ))}
        ...
        {activeTab === 'examples' && (
          <div className="space-y-3">
            <p className="text-xs font-semibold text-slate-700 mb-1">
              Select an authoritative test requirement to test the engine:
            </p>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
              {samplePrompts.map((sample, idx) => (
                <button
                  ...
                  className="text-left p-3 rounded-xl border border-slate-200 hover:border-blue-400 hover:bg-blue-50/40 transition group flex flex-col justify-between"
                >
                  <div>
                    ...
                    <p className="text-xs font-semibold text-slate-800 group-hover:text-blue-700 mb-1">
                      {sample.title}
                    </p>
                  ...
                  <span className="text-[11px] font-semibold text-blue-600 mt-2 flex items-center">

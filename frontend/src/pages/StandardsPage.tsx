...
import { Search, Filter, BookOpen, AlertCircle, RefreshCw, ChevronRight } from 'lucide-react';
...
export const StandardsPage: React.FC = () => {
  ...
  // Keep search params in sync if changed from external navbar search
  useEffect(() => {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6 sm:space-y-8">
        ...
        <div className="space-y-2">
          <nav className="flex items-center space-x-2 text-xs text-slate-500 font-medium">
            ...
            <span className="text-slate-800 font-bold">Standards Library</span>
          </nav>
          ...
          <div className="border-b border-slate-200 pb-5">
            <div className="inline-flex items-center space-x-2 text-[10px] font-bold uppercase tracking-wider text-[#0067C5] bg-blue-50 border border-blue-200 px-2.5 py-0.5 rounded mb-2">
              <BookOpen className="w-3.5 h-3.5 text-[#0067C5]" />
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
              <div>
                <h1 className="text-2xl sm:text-3xl font-black text-[#0B192C] tracking-tight">
                  Indian Standards (BIS) Directory
                </h1>
                <p className="text-xs sm:text-sm text-slate-600 mt-1">
                  Search and inspect technical specifications, classifications, and regulatory certification marks indexed in the ManakSetu registry.
                </p>
              </div>
              <div className="text-xs font-bold text-[#0067C5] bg-white border border-slate-200 px-3 py-1.5 rounded-lg shadow-2xs w-fit">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-4 shadow-sm flex flex-col md:flex-row items-center gap-3">
          ...
          <div className="relative flex-1 w-full">
            ...
            <input
              ...
              placeholder="Search by IS number (e.g. IS 2082), title, or keywords..."
              className="w-full pl-10 pr-4 py-2 text-xs rounded-xl border border-slate-300 focus:outline-none focus:border-[#0067C5] focus:ring-2 focus:ring-blue-100 text-slate-800"
            />
          </div>
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-12 text-center text-slate-500">
            ...
            <p className="text-xs text-slate-400">
              Try adjusting your search terms or resetting the status/classification filters.
            </p>
          </div>
        ) : (
